...
usage() {
  echo "Usage: $0 <40-character-infra-commit> <syntax|check|apply|bootstrap-check|bootstrap-apply|ingress-check|ingress-apply|logs-check|logs-apply|logs-packages-apply|media-bootstrap-apply|media-check|media-apply> [inventory]" >&2
  exit 2
}
...
case "$mode" in syntax|check|apply|bootstrap-check|bootstrap-apply|ingress-check|ingress-apply|logs-check|logs-apply|logs-packages-apply|media-bootstrap-apply|media-check|media-apply) ;; *) usage ;; esac
...
case "$mode" in
  ...
  media-bootstrap-apply) exec ansible-playbook -i "$inventory" "$playbook" --tags rf-dev-media-bootstrap --diff ;;
