...
SOURCE_SECRET="${SOURCE_SECRET:-playsay-postgres-worksheet-import}"
TARGET_SECRET="${TARGET_SECRET:-playsay-worksheet-import-db}"
TARGET_NAMESPACES="${TARGET_NAMESPACES:-playsay-dev jenkins}"
DB_NAME="${DB_NAME:-worksheet_import}"
DB_USERNAME="${DB_USERNAME:-worksheet_import_app}"
...
usage() {
  cat <<USAGE
Usage:
  SOURCE_NAMESPACE=playsay-data SOURCE_SECRET=playsay-postgres-worksheet-import TARGET_NAMESPACES="playsay-dev jenkins" $0

Ensures the CloudNativePG worksheet-import role password secret exists in the data namespace,
...
kubectl -n "$SOURCE_NAMESPACE" label secret "$SOURCE_SECRET" \
  app.kubernetes.io/name=playsay-worksheet-import-db \
  app.kubernetes.io/managed-by=playsay-infra \
  playsay.io/component=worksheet-import \
  cnpg.io/reload=true \
  --overwrite >/dev/null
