...
- `online.honey.school` returns HTTP 200, API readiness returns 200 and unauthenticated `/api/me` returns the expected 401 after the digest-pinned release rollout;
...
- every deployed prod ArgoCD application is `Synced/Healthy`, no prod pod is unready, and web runs `web-release-1.001.00-134` at digest `sha256:2304116ac078bb9d718208a31e6df349a527a09c23091e8f64b99576ad637437`;
- browser smoke observed silent and interactive authorization only at `key.honey.school`, with redirect URI `https://online.honey.school/auth/callback`; the compiled bundle contains the honey issuer and no retired ops issuer;
- the AX41 edge sends `/collab/ws` to the digest-pinned collaboration service and `/livekit/` to prod signaling; invalid upgrade probes fail closed with HTTP 403 and 401 respectively instead of returning the SPA.

Remaining acceptance gates are a human Maria login, one reviewed-student login,
rendering the transferred materials with their preserved passwords, owner-side
branch-protection confirmation, stabilization monitoring and the mandatory final
encrypted off-host backup before the old VPS is deleted. Separate prod provider
credentials are still required before email/payment integrations are enabled;
their absence does not block login or material access.
