...
Platform `00e53fe7` is deployed by web #310 through infra GitOps `c60cddf`, immutable digest `sha256:ad07df2a9e7fd56ba0ea7e5710346a4090abbc00eb2f94cea36f359bcf29b0a3`. The ready dev deployment uses that digest. Four focused auth regression cases plus existing auth tests passed locally (14 total); lint and build passed. Actual fresh login to an owned active lesson passed on both direct/RF dev origins, preserving `?probe=geoip#section` and displaying prejoin. Fixtures were cleaned by the harness. Jenkins #310 finished SUCCESS in every stage, including UI smoke. Independent API cleanup verification confirmed lesson 404 and material ARCHIVED; all 21 dev ArgoCD applications are Synced/Healthy. The isolated browser and MMDB probe processes were closed.
