#!/usr/bin/env python3
"""Add only production location includes to existing routes; preserve other bytes."""
import argparse
...
HOSTS = {'honey.school', 'online.honey.school', 'key.honey.school'}
...
def render(source):
    ...
    for i, section in enumerate(sections):
        ...
        if len(names) != 1 or section.count('    location / {\n') != 1:
            raise ValueError('Unexpected production vhost structure')
        count += 1
        if INCLUDE not in section:
            sections[i] = section.replace('    location / {\n', '    location / {\n' + INCLUDE)
    if count != 6:
        raise ValueError('Expected exactly four HTTP/HTTPS production vhosts')
...
if __name__ == '__main__':
    ...
    if before == after:
        ...
        try:
            ...
            raise SystemExit('Production route validation failed; previous routes restored')
        print('changed')
