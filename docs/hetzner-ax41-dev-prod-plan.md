...
- [ ] **T4** Bootstrap independent Ubuntu/k3s/ArgoCD environments from Git; dev and prod receive separate CIDRs, secrets, databases, MinIO and Keycloak. Independent k3s, ArgoCD and Sealed Secrets controllers are complete; prod application/data apps remain pending.
- [ ] **T5** Create/protect `release/1.001.00`, build immutable `rel_1.001.00-N` images and promote only their digests through the matching infra release branch. Do not deploy prod from `develop`, `main` or `hotfix/*`. Both release branches exist; the serialized Jenkins candidate build is running and the aborted API module must be retried with its original release parameters.
