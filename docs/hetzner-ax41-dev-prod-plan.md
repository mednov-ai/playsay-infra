...
- [ ] **T9** Verify exact prod counts, Maria and one student login, every material/object, Keycloak redirects, API/WebSocket and LiveKit/TURN before accepting user traffic. SQL counts, exclusion/history/orphan checks and byte-for-byte SHA-256 for all 51 objects pass; authenticated human login, rendered-material, WebSocket and forced-TURN checks remain.
