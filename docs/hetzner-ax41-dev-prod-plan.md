...
- `playsay-prod`: clean production cluster, separate state and integrations, built only from a protected fixed-width release branch named `release/NN.NNN.NN` (for example `release/01.002.00`).
...
Changing an image tag is not a production deployment. A release starts from a protected fixed-width branch such as `release/01.002.00` in `playsay-platform`; Jenkins labels its builds, publishes immutable digests, and writes only those digests to the matching protected `playsay-infra` release branch. The first counter is the major product generation, the middle counter increments for ordinary releases and resets the final counter, and the final counter increments only for fixes. Numeric release builds never update dev. Jenkins has no production cluster or database credentials: an operator reviews the completed infra release diff, approves/runs migrations, manually syncs prod, verifies smoke, and records the accepted version in `argocd-apps/prod/current-release.txt`. The deployed release branch remains available while that version is running. `main`, `develop`, free-form `release/*` names and direct `hotfix/*` builds cannot deploy prod; a hotfix must become a new fixed-width release branch. A prod rollback points back to the previous known-good digest or previous retained release branch; images are not rebuilt during rollback.
...
- [ ] **3.2** Implement the `10.60.0.0/24` bridge, nftables default-deny policy, NAT and the explicit prod/dev LiveKit/TURN port split. The libvirt bridge and NAT are active; prod uses coturn `3478` plus relay UDP `49160-49200` and LiveKit TCP `7881` plus direct UDP `50000-50020`. UFW/DNAT and the persistent libvirt network hook are applied; external TCP and authenticated forced UDP relay pass. The non-overlapping dev public range remains a later change.
...
- [x] **10.2** Issue/verify certificates only after DNS resolves and configure exact host routing; keep all ops routes VPN-only despite public DNS names. The SAN certificate is valid through `2026-10-18`; no public ops route was created.
- [x] **10.3** Run pre-cutover smoke through explicit host resolution without changing user traffic: web, API, Keycloak redirect/origin, WebSocket, MinIO, LiveKit and forced TURN relay. Browser smoke records `key.honey.school` with callback `online.honey.school/auth/callback`; invalid collaboration and LiveKit handshakes reach the intended services and fail closed with `403`/`401` rather than returning SPA HTML.
