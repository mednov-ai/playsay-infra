...
- [ ] **T9** Verify exact prod counts, Maria and one student login, every material/object, Keycloak redirects, API/WebSocket and LiveKit/TURN before accepting user traffic. SQL counts, exclusion/history/orphan checks, byte-for-byte SHA-256 for all 51 objects and external forced-TURN relay pass; authenticated human login, rendered-material and application WebSocket checks remain.
