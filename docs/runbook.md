...
AX41 at `65.109.55.110` serves `honey.school` production from `release/01.004.04`: the Ubuntu 24.04 physical host, healthy mdadm RAID1/ext4, KVM/QEMU/libvirt, OpenTofu, firewall, WireGuard and VPN-only Cockpit baseline are active. The NAT-backed `playsay-prod` (`10.60.0.20`, 8 vCPU/38 GiB), `playsay-dev` (`10.60.0.30`, 2 vCPU/10 GiB) and `playsay-ci` (`10.60.0.40`, 4 vCPU/8 GiB) guests are active and protected by separate OpenTofu states. The 14 declared guest vCPUs moderately overcommit the host's 12 logical CPUs, while fixed guest memory remains 56 GiB and leaves about 6 GiB for the host. Prod/dev run independent k3s, ArgoCD and Sealed Secrets controllers. CI runs only k3s, Sealed Secrets and the Git/JCasC-defined Jenkins controller; it has no product workloads, ArgoCD, Keycloak or MinIO. Production has 18 ArgoCD applications; all are `Synced/Healthy` at infra release commit `442c99474f968268120e130472b2646b17fe1f80`. Web, keyboard, API, collaboration signaling, LiveKit signaling, forced TURN, transactional Mailjet email and production monitoring reach AX41. Canonical Keycloak is `ops.*/keycloak`; the explicit `key.*/keycloak` compatibility path remains only until owner-operated login/material acceptance. Both GitHub webhooks use `hooks.honey.school`. Payment remains disabled until independent production credentials are supplied. Object Storage remains a post-stabilization state/backup task, not a blocker to this cutover. Release evidence: `migrations/ax41/evidence/20260819-release-01.004.04.md`; prior capacity evidence remains `migrations/ax41/evidence/20260727-release-1.001.08-capacity.md`.
...
- login theme: `playsay`, stored in Git under `helm-charts/keycloak/themes/playsay` and mounted into Keycloak by the wrapper chart as a ConfigMap volume. Theme caches are disabled in dev. The theme serves the approved Honey School main/reverse SVG logos, favicon and local Quicksand 400/500/600 fonts from its ConfigMap; the technical theme id remains `playsay`. The page syncs color mode with the SPA: `playsay_theme=light|dark|system` in the authorize URL sets `data-playsay-theme` and `data-playsay-resolved-theme` on `<html>`, while direct login links and `system` follow `prefers-color-scheme`; language dropdown links preserve the current `playsay_theme` parameter. Login is Passkey-first: the large passkey action is rendered before credentials, the native picker is attempted once per Keycloak authentication session with `mediation=optional`, and username/password are initially collapsed behind a secondary action. Cancelling the picker keeps the password form collapsed; unsupported WebAuthn or an unexpected error expands it, while a failed password reload starts expanded and never relaunches the picker. The page keeps warm Honey School background accents, one hero heading and a quiet localized return link to `https://honey.school/`. The custom WebAuthn registration page never asks for a credential label: it submits a localized default. Desktop starts registration immediately with a visible fallback button; iPhone/iPad Safari shows the button first so `navigator.credentials.create` runs inside a user gesture. All custom visible theme texts live in Keycloak message bundles for `ru`, `en`, `de`, and `fr`; frontend `ui_locales` and the Keycloak language dropdown must change both `<html lang>` and visible copy.
...
Before rollout run `./scripts/test-keycloak-passkey-theme.sh`; it rejects the label prompt, verifies the iPhone/iPad registration gesture branch, enforces Passkey-before-password ordering and checks the once-per-authentication-session guard. Then run `helm dependency build helm-charts/keycloak`, `helm lint helm-charts/keycloak -f helm-charts/keycloak/values-dev.yaml`, and `helm template keycloak helm-charts/keycloak -f helm-charts/keycloak/values-dev.yaml`; do not commit the downloaded `charts/*.tgz`. After rollout verify the pod image/readiness, discovery document, realm WebAuthn fields through the Admin API, password login, the once-per-browser setup offer, first and additional AIA registration, in-app rename/delete, cancellation, password reset fallback, and that a redirect without a newly persisted credential is reported as failure. Use Chromium's virtual authenticator for repeatable add/login/rename/delete and Passkey-first auto-login smoke. In mobile WebKit verify the Passkey CTA precedes the collapsed password form, cancellation stays on that screen, the secondary password action reveals and focuses the username field, and a wrong password returns with the fallback expanded without another picker. On real iPhone Safari verify that **Add passkey → Keycloak continuation button → native Face ID/Touch ID/passcode sheet → profile with one extra key** completes without a silent bounce; then sign out, verify the native login picker appears once, cancel it and confirm the password form stays collapsed and the explicit Passkey button still works. Also accept one real Touch ID or Windows Hello registration/login. These real-device checks are a required production regression gate; release `01.002.00` passed them through owner acceptance before promotion.
