...
The production web image active at the first TLS 1.2 cutover still placed a render-blocking Google Fonts stylesheet in its HTML. Chrome therefore opened an additional TLS 1.3 connection to `fonts.googleapis.com`, which remained capable of blocking first paint on affected Russian networks even though every AX41 listener was TLS 1.2-only. As a temporary bridge, the AX41 `online` routes remove the two Google preconnect tags and replace that exact stylesheet URL with an empty local data stylesheet. The SPA uses its sans-serif fallback until a release image bundles Manrope locally. Remove the nginx substitutions only after that image is promoted and owner acceptance succeeds without VPN; do not treat the bridge as the permanent font delivery design.
