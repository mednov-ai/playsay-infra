...
AX41 at `65.109.55.110` serves `honey.school` production from `release/01.004.03`: the Ubuntu 24.04 physical host, healthy mdadm RAID1/ext4, KVM/QEMU/libvirt, OpenTofu, firewall, WireGuard and VPN-only Cockpit baseline are active. The NAT-backed `playsay-prod` (`10.60.0.20`, 8 vCPU/38 GiB), `playsay-dev` (`10.60.0.30`, 2 vCPU/10 GiB) and `playsay-ci` (`10.60.0.40`, 4 vCPU/8 GiB) guests are active and protected by separate OpenTofu states. The 14 declared guest vCPUs moderately overcommit the host's 12 logical CPUs, while fixed guest memory remains 56 GiB and leaves about 6 GiB for the host. Prod/dev run independent k3s, ArgoCD and Sealed Secrets controllers. CI runs only k3s, Sealed Secrets and the Git/JCasC-defined Jenkins controller; it has no product workloads, ArgoCD, Keycloak or MinIO. Production has 18 ArgoCD applications; all are `Synced/Healthy` at infra release commit `87debcc3846c66f641dd94ab340f037e03c27162`. Web, keyboard, API, collaboration signaling, LiveKit signaling, forced TURN, transactional Mailjet email and production monitoring reach AX41. Canonical Keycloak is `ops.*/keycloak`; the explicit `key.*/keycloak` compatibility path remains only until owner-operated login/material acceptance. Both GitHub webhooks use `hooks.honey.school`. Payment remains disabled until independent production credentials are supplied. Object Storage remains a post-stabilization state/backup task, not a blocker to this cutover. Release evidence: `migrations/ax41/evidence/20260818-release-01.004.03.md`; prior capacity evidence remains `migrations/ax41/evidence/20260727-release-1.001.08-capacity.md`.
