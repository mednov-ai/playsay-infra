...
The same RF-access incident showed a fresh Chrome/incognito client receiving HTML but stalling on larger JS/CSS transfers over both HTTP/2 and HTTP/1.1. Live TCP state for the affected connections confirmed a PMTU black hole rather than an application or HTTP-version failure: 21 full-size segments were lost, about 130 KB remained queued, congestion window collapsed to one segment, and retransmission timeouts backed off to 15–30 seconds while no usable ICMP Packet Too Big response arrived. The AX41 edge therefore enables TCP Packetization-Layer Path MTU Discovery with `net.ipv4.tcp_mtu_probing=2` and conservative initial `net.ipv4.tcp_base_mss=1024`, allowing new connections to start below the broken path MTU and probe upward. HTTP/1.1 remains a temporary conservative compatibility layer through `edge_http2_enabled: false`; re-enable HTTP/2 only after cache-empty acceptance succeeds on the affected carrier network.
