...
During this rollback overlap, the legacy application hosts `online.play-and-say.ru` and `key.play-and-say.ru` must select the canonical dev issuer `https://dev.ops.honey.school/keycloak/realms/playsay`. The restored old/new dev realms have matching public signing keys. The dev Helm overrides for both `api-gateway` and `keyboard-service` must validate that issuer while reading JWKS from the cluster-local Keycloak service; otherwise the PKCE token exchange can succeed but authenticated `/api/me` returns `401` on the affected app host. Keep the frontend compatibility mapping and both backend issuer overrides covered by tests/render checks, verify a full PKCE login plus authenticated `/api/me` on both legacy app hosts, and remove the bridge together with the legacy app-host DNS and old VPS—not earlier. This bridge does not change or overwrite the root `play-and-say.ru` site.
...
The production workaround is a narrow Russian TLS ingress on `146.103.126.15`, managed by `ansible/playbooks/rf-edge.yaml`. Only `online.honey.school`, `key.honey.school` and `ops.honey.school` move to exact A records for that ingress. It terminates client TLS 1.2 and proxies the original hostname over certificate-verified TLS 1.2 to the AX41 address `65.109.55.110`; `ops.honey.school` exposes only `/keycloak/`, while its root and operational UIs remain unavailable on this ingress. `dev.*`, `jenkins.honey.school` and `hooks.honey.school` stay directly on AX41. Before DNS cutover, forced-resolution smoke through `146.103.126.15` must return HTTP 200 for the full web JS, full keyboard JS and Keycloak discovery. After cutover, repeat the same checks from an affected Russian no-VPN connection before accepting the route.
...
The current shared `honey.school` certificate is staged under `/etc/playsay/rf-edge-tls` only to make the emergency cutover safe. After the three exact DNS records propagate, issue an independent Certbot certificate for `online.honey.school`, `key.honey.school` and `ops.honey.school` on the Russian ingress, switch the Ansible certificate directory to its `/etc/letsencrypt/live/...` path, verify unattended renewal and split the remaining AX41 SAN renewal away from those three names. Do not leave the copied certificate as the long-term renewal design.
