...
Dev central-log storage is declared as an independent VictoriaLogs deployment in the existing monitoring-lite chart: NodePort 32089, 7-day retention, 10 GiB PVC, 128 MiB requested/512 MiB maximum memory and 500m CPU maximum. NodePort 32086 remains collaboration. This delivery adds storage only; public log routes and Selectel collection remain disabled until authenticated ingestion and privacy checks pass. Query access must remain VPN-only; do not expose NodePort 32089 publicly. Rollback disables `victoriaLogs.enabled` through GitOps only after stopping its collector; preserve the PVC when diagnostic evidence is still required.
