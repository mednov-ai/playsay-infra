...
Production hotfix `release/01.005.03` restores compact reusable lesson links on both `online.honeyschool.ru` and `online.honey.school` and preserves the originating application domain through login and password recovery. Platform `develop` and the numeric release both use source commit `2a3987a6556fb56cbc13fbaa7a691c8fdbc0329b`; immutable API and web digests are recorded in the ready candidate. A fresh four-database logical backup passed SHA-256 and archive-list validation, and the bounded production Liquibase Job applied exactly the reviewed additive compact-link changeset before rollout with zero pending changesets afterward. All 19 ArgoCD applications reached `Synced/Healthy` at infra candidate `0da1a441d63af3c4b2913d253c2bb8ee1c4f2e59`. Browser acceptance on both origins confirmed public compact resolution, fragment removal, origin-specific OIDC callbacks and keyboard-accessible forgot-password continuity. The reported student reset canary reached `CODE_SENT` and Mailjet `DELIVERED`; inbox placement and completion remain owner-confirmed. Evidence: `migrations/ax41/evidence/20260830-release-01.005.03.md`. Roll back the production root to `release/01.005.02`; retain the additive nullable columns, and rotate the disclosed incident link through the teacher-authorized application flow.
