...
The same RF-access incident also showed a fresh Chrome/incognito client receiving HTML but repeatedly aborting the large application JS/CSS responses over HTTP/2, then retrying tiny byte ranges after roughly 30 seconds. Cached profiles hid the failure because those assets were already local. The AX41 edge therefore sets `edge_http2_enabled: false` and advertises HTTP/1.1 only while retaining TLS 1.2. Keep this compatibility mode until cache-empty acceptance succeeds on the affected carrier networks; re-enable HTTP/2 only through the Ansible variable and repeat the no-VPN test.
