...
During this rollback overlap, the legacy application hosts `online.play-and-say.ru` and `key.play-and-say.ru` must select the canonical dev issuer `https://dev.ops.honey.school/keycloak/realms/playsay`. The old cluster's backend already validates that issuer, and the restored old/new dev realms have matching public signing keys. A legacy frontend that falls back to `https://ops.play-and-say.ru:18443/keycloak/realms/playsay` completes the token exchange but receives `401` from `/api/me`. Keep the compatibility mapping covered by frontend unit tests, verify a full PKCE login plus authenticated `/api/me`, and remove it together with the legacy app-host DNS and old VPS—not earlier. This bridge does not change or overwrite the root `play-and-say.ru` site.
