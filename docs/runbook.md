...
RF dev/GeoIP work is currently local preparation only: see [RF dev preparation](rf-dev-preparation.md) for the environment map, independent TURN constraints, selected log design and protected provider-token references. Do not treat it as deployed state or as permission to change production. The prepared log path is Selectel Fluent Bit → authenticated `dev.ops.honey.school/victoria-logs/insert/` → VictoriaLogs in `playsay-dev`; its independent backend and collector flags stay off until a coordinated dev delivery.
