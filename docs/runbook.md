...
Newest accepted production runtime is `release/01.007.01`, candidate `44847b3ee00f888e39c78c606f83afe25036dca2`. This hotfix restores teacher-entered YouTube metadata recovery and updates only API, web and media images. All seven schema checks passed without migrations; all 19 applications are Synced/Healthy and browser diagnostics passes on both online origins. See [production release evidence](../migrations/ax41/evidence/20260907-release-01.007.01-youtube-metadata.md). Earlier runtime paragraphs below are historical. Production GeoIP navigation remains enabled through its separately scoped production play; application deployment does not change the shared nginx flag.

RF dev DNS, HTTPS/auth ingress, independent TURN and authenticated central log collection are active. Two-browser synthetic-media verification through RF dev passes camera/audio, collaboration, screen share and reconnect after the scoped dev SNAT and LiveKit uplink fixes. Real RF last-mile without VPN and long-duration acceptance remain open. See [RF dev preparation](rf-dev-preparation.md) for exact refs, evidence, isolation and provider-token requirements. Dev GeoIP and its daily updater are active; production GeoIP was independently enabled after release 01.007.00.
...
AX41 at `65.109.55.110` serves `honey.school` production from `release/01.007.03`: the Ubuntu 24.04 physical host, healthy mdadm RAID1/ext4, KVM/QEMU/libvirt, OpenTofu, firewall, WireGuard and VPN-only Cockpit baseline are active. The NAT-backed `playsay-prod` (`10.60.0.20`, 8 vCPU/38 GiB), `playsay-dev` (`10.60.0.30`, 2 vCPU/10 GiB) and `playsay-ci` (`10.60.0.40`, 4 vCPU/8 GiB) guests are active and protected by separate OpenTofu states. The 14 declared guest vCPUs moderately overcommit the host's 12 logical CPUs, while fixed guest memory remains 56 GiB and leaves about 6 GiB for the host. Prod/dev run independent k3s, ArgoCD and Sealed Secrets controllers. CI runs only k3s, Sealed Secrets and the Git/JCasC-defined Jenkins controller; it has no product workloads, ArgoCD, Keycloak or MinIO. Production has 19 ArgoCD applications; all are `Synced/Healthy` at the accepted numeric branch. Web, keyboard, API, worksheet import, collaboration signaling, LiveKit signaling, forced TURN, transactional Mailjet email and production monitoring reach AX41. Canonical Keycloak is `ops.*/keycloak`; the explicit `key.*/keycloak` compatibility path remains only until owner-operated login/material acceptance. Both GitHub webhooks use `hooks.honey.school`. Payment remains disabled until independent production credentials are supplied. Release evidence: `migrations/ax41/evidence/20260908-release-01.007.03-jvm-security.md`; prior capacity evidence remains `migrations/ax41/evidence/20260727-release-1.001.08-capacity.md`.

Current accepted production is `release/01.007.03`, with all 19 ArgoCD applications Synced/Healthy. This security release adds retained OWASP Dependency-Check evidence across active JVM build and service modules, upgrades Keycloak to 26.7.3, and carries owner-accepted temporary exceptions for CVE-2026-53914 and CVE-2026-62380 until 2026-10-08 00:00 UTC. Its affected immutable JVM images are built from platform commit `ea2b1c38c2948523990cd0d4c036aeec63625449`; the ready infra candidate is `77f646028e35a119aa5b9f87cbdaba65aaa6fba3`. Verify the live revision before any subsequent delivery.
...
The old VPS has only the short paid overlap remaining. Final source bundle `playsay-final-vps-v2-20260721T083819Z` has been copied off the VPS and fully decrypt/checksum/archive verified. Keep the VPS and old Jenkins available only as rollback through owner acceptance; delete neither without explicit owner approval. The old hosts `146.103.126.15` and `89.124.113.223` are never a current dev target: agents must not connect to, diagnose, restart, build on or deploy to them unless the owner explicitly requests a named legacy rollback action in the current chat. Amnezia is not migrated and ends with the VPS. Agent SSH does not use WireGuard: connect directly to the public AX41 host and use it as the jump host for the NAT-backed guests. WireGuard is only for a human operator's browser access to the protected ops zone, including Cockpit, ArgoCD, monitoring and Jenkins, and for its split DNS.

The current SSH route from this workstation uses `/Users/evgeniymednov/.ssh/play_and_say_vps_ed25519`. Connect to the physical host directly with `root@65.109.55.110`. Reach every guest through that public jump host as `playsay`: production at `10.60.0.20`, dev at `10.60.0.30`, and CI at `10.60.0.40`. For example, replace `<guest-ip>` below with the required guest address:
...
Do not activate or require WireGuard for this SSH route. A failed VPN or inactive tunnel is therefore not an SSH diagnostic prerequisite.
...
### HTML-game upload request limits and recovery

HTML-game uploads have a 20 MiB product limit enforced by api-gateway. Only `POST /api/materials/{materialId}/assets/html-games` and `POST /api/schedule/lessons/{lessonId}/html-game-page` receive a 21 MiB multipart envelope in the web-app nginx, AX41 edge, and RF edge. Generic `/api`, material create/update, External Activity resolution, URL import, media, identity, hooks, and ops routes retain their previous limits. The application-level oversize response is HTTP `413` with `errorCode=MATERIAL_HTML_GAME_TOO_LARGE`; an HTML response with 413 identifies an nginx envelope rejection and must still settle the browser loading state with localized retry guidance.

Before rollout, render or inspect all three desired-state configurations and verify that `client_max_body_size 21m` occurs only in the two-path regular expression. Run the committed configuration contract test, then validate the web image configuration with `nginx -t` in its image and render both Ansible templates with the normal AX41/RF inventories in check mode. Do not print inventory secrets. In browser Network tools, record only origin, request path class, method, request size, response status/content type, responding layer, and visible result; never retain authorization headers, bodies, material identifiers, or learner content.

The safe dev acceptance order is: deploy api-gateway first, then web-app, then AX41 and RF desired-state changes through Jenkins/ArgoCD/Ansible as already authorized for that release. Test the two supplied sub-20-MiB games in reusable material and live lesson, a synthetic file exactly at 20 MiB, and one byte over. Accepted requests must reach api-gateway; oversize requests must show a recoverable localized banner and leave material/lesson state unchanged. Validate `https://www.liveworksheets.com/worksheet/en/english-second-language-esl/710637` as an External Activity: classification and persistence must succeed without an outbound provider fetch. Do not widen material JSON routes unless sanitized browser evidence separately proves an nginx rejection there.

Rollback in reverse order through GitOps: restore the previous edge/RF templates and web image, then restore api-gateway. Validate rendered nginx before reload and repeat one small rejected/accepted request on dev. Rollback does not delete uploaded assets, materials, lesson state, or External Activity links.
...
Database migration approval is state-based, not Git-diff-based. From an exact checkout of the numeric platform release, expose the `playsay-prod` guest's `kubectl` as a local executable and run `scripts/ci/check-production-schema-convergence.sh release/NN.NNN.NN`. The checker reads `scripts/ci/production-database-workloads.json`, runs each enabled workload's exact candidate changelog in a bounded status-only Job, and fails closed on an invalid release ref, disallowed Secret, checksum error, pending changeset, Job failure, or non-empty final status. Unchanged application code or image digest never permits this check to be skipped.

When an approved workload has a reviewed pending set, first create and verify a restorable database backup, record only its protected path and checksum, then run `PLAYSAY_PRODUCTION_MIGRATION_CONFIRM=APPLY:release/NN.NNN.NN:<module> scripts/ci/run-production-liquibase-job.sh update <module> <changelog-dir> <approved-db-secret> release/NN.NNN.NN <expected-pending-file>`. The expected file contains one exact `id::author::path` identity per line. The launcher must report an exact pre-update match and zero pending changes afterward; it cleans only its own Job/ConfigMap and never prints or decodes Secret values. Re-run the all-workload convergence checker before ArgoCD promotion. Schema rollback is forward-only by default; database restore is a separate destructive recovery decision made from the verified pre-change backup.
...
The accepted replacement for the temporary old-VPS workaround is a dedicated Selectel Russian ingress at `94.102.89.213`, managed by `ansible/playbooks/rf-edge.yaml`. It is an Ubuntu 24.04 VM with 1 vCPU, 2 GiB RAM and a 25 GiB system disk. Host nginx, Certbot and UFW own ingress, and a bounded coturn service may additionally carry one approved `.ru` classroom canary under the shared-host contract below. The firewall permits only SSH, public HTTP/HTTPS, and that declared TURN surface. The nginx HTTP context and every generated TLS vhost remain TLS 1.2-only. Client traffic is proxied over certificate-verified TLS 1.2 to the AX41 address `65.109.55.110`.
...
The direct production origins `online.honey.school` and `key.honey.school` remain on AX41 and must not be changed when the Russian aliases are activated. On 2026-07-25 the exact REG.RU A records `online.honeyschool.ru -> 94.102.89.213` and `key.honeyschool.ru -> 94.102.89.213` became authoritative, and both Selectel proxy routes were activated over HTTPS. Each preserves the corresponding `.school` Host/SNI upstream, verifies AX41 TLS and rewrites absolute application/websocket URLs back to the `.ru` alias only in static HTML, JavaScript and CSS payloads. It must never rewrite `application/json`: API responses can intentionally contain both supported origins, including the `.ru` and `.school` lesson-link copy choices. Public full-transfer smoke returned the complete web JS (`1,157,070` bytes), web CSS (`226,388` bytes), keyboard JS (`356,669` bytes) and keyboard CSS (`64,663` bytes). HTTP redirects to HTTPS, TLS 1.2 succeeds and TLS 1.3 is rejected for both aliases.
...
### Selectel classroom collaboration route

The independent classroom collaboration/Yjs route is specified in [`rf-edge-collaboration-route.md`](rf-edge-collaboration-route.md). Production desired state is `collaboration.regionalRouting.mode=rf-two-hop`; dev remains `off`, and production `off` is rollback only. Do not infer collaboration behavior from the LiveKit signaling or media flags. Before application delivery, require zero active lessons, run the reviewed RF playbook in check mode, apply the exact `/collab/ws` edge support, and require a second check with `changed=0`. Run `scripts/validate-rf-classroom-collaboration-route.sh --static`, `scripts/validate-rf-collaboration-config.sh`, `scripts/collect-classroom-collaboration-propagation.sh --self-test`, and `scripts/collect-production-media-window.sh --self-test`; then verify nginx/collaboration health, unchanged listeners/UFW, and the direct/RF unauthenticated authorization boundary without a token-bearing query.

After the application release is delivered, prove fresh `.ru` tokens select regional `/collab/ws` and `.school` remains direct, then immediately use two no-VPN browsers, close unrelated tabs, measure sanitized drawing propagation, and collect authenticated production metrics for the identical UTC window. Accept only one two-participant lesson when p95 is below 300 ms, every update is below one second, and reconnect/stale/backpressure/snapshot/restart gates are all zero. On any breach, set only collaboration mode to `off` for fresh tokens and let old RF sockets drain; do not restart nginx/collaboration, touch LiveKit/coturn, or modify identity/persistence. Never copy raw collaboration access lines: both exact `/collab/ws` ingress locations have access logging disabled because their query contains a token.
...
The 2026-09-09 lesson exposed a false-zero RF allocation metric: LiveKit recorded two Selectel TURN connections and the RF host had six UDP sockets in the configured relay range, while the collector exported zero because it parsed the `ss -H -lun` peer-address column. The corrected collector uses the native bounded source-port filter, captures a successful socket query before opening the temporary textfile, and preserves both metric names with an identical integer value. Never accept exported zero while stable LiveKit evidence says `connectionType=turn` through the RF edge.

After any collector delivery, run `/usr/local/sbin/playsay-validate-rf-edge-media-relay --monitoring`; the wider shared-host preflight remains `/usr/local/sbin/playsay-validate-rf-edge-media-relay --configuration`. Monitoring mode refreshes the textfile and compares both allocation metrics with an independent bounded kernel query without printing addresses. During a stable `.ru` TURN session require equal positive values; after every allocation drains require equal zero values. A missing, duplicated, malformed, negative, unequal, or persistently stale sample invalidates preflight. The diagnostic kernel query is `ss -H -lun 'sport >= :49152 and sport <= :49251'`; retain only the aggregate line count in evidence.

For a collector/validator-only hotfix, invoke `scripts/apply-rf-edge-release.sh --check --monitoring-only` and then the identically scoped `--apply --monitoring-only`. The tag installs exactly those two files and has no service notification. Reject a full-play preview that contains unrelated drift or a service handler; do not use a broad reconciliation as a shortcut for this monitoring repair.
...
Run repository syntax from any branch, then check and apply only from the clean pushed numeric release as described above. Immediately rerun check after apply and require `changed=0`; any second-pass change blocks canary enablement. The live allocation validator reads the REST secret inside its root-only process, passes only five-minute derived credentials to `turnutils_uclient`, suppresses raw probe output, and prints sanitized PASS labels:

```bash
scripts/apply-rf-edge-release.sh --syntax

ssh -i /absolute/path/to/key root@RF_EDGE \
  /usr/local/sbin/playsay-validate-rf-edge-media-relay --configuration

# Zero-active-lesson window only; routing remains off.
ssh -i /absolute/path/to/key root@RF_EDGE \
  /usr/local/sbin/playsay-validate-rf-edge-media-relay --allocations
```

Preflight requires DNS and certificate name/chain/expiry, `nginx -t`, active nginx/coturn, default-deny UFW, the exact listener set, loopback-only exporter, Selectel-to-AX41 reachability, invalid-credential rejection, real REST-secret allocations and bidirectional data over UDP/TCP/TLS, shared-host headroom, and a rehearsed flag-first rollback. An open socket is not allocation evidence. Then run two `.ru` browsers in relay-only mode with camera, microphone, bidirectional subscriptions, and one screen-share start/receive/stop cycle; prove the selected relay address is the RF edge without retaining candidates or addresses in evidence.

The first 2026-09-01 no-VPN canary failed as `SIGNALING_ROUTE_UNAVAILABLE`: LiveKit observed relay-class candidates while room tokens still named direct `.school` signaling; LiveKit recorded signal-source closures and replacement loops, Selectel remained healthy, and VPN restored both clients. Evidence is in `migrations/ax41/evidence/20260901-rf-classroom-signaling-canary-failure.md`. A later canary proved RF/AX41 two-hop signaling was reachable but failed as `REGIONAL_MEDIA_NOT_SELECTED_OR_AUTHENTICATED`: active participants had zero observed RF TURN allocations, TURN authentication failures, repeated resumes/source closes, RTT p95 about `2.3 s`, jitter p95 about `87 ms`, and loss p95 about `4.6%`, while the shared RF host remained healthy. A protected comparison returned only `match` for the production API and coturn secrets, so rotation was not required. Evidence is in `migrations/ax41/evidence/20260901-rf-classroom-media-canary-failure.md`.

Application support is deployed first with `livekit.regionalRelay.signalingMode=rf-two-hop` only after its independent signaling gate passes and with `livekit.regionalRelay.mediaMode=off`. The temporary legacy value `livekit.regionalRelay.mode=rf-origin-relay` maps to the same signaling-on/media-off state and must not enable media. Before any edge apply, run `scripts/validate-rf-classroom-signaling-route.sh --static`; after the reviewed RF release, run its `--probe` mode and require bounded unauthenticated responses from both `.ru` two-hop and `.school` direct paths. Then run RF Ansible check/apply during a zero-active-lesson window and repeat check with `changed=0`; nginx and coturn must remain healthy. The explicit RF `/livekit` locations preserve the path and WebSocket Upgrade over verified TLS 1.2 to `online.honey.school`, disable buffering and never perform response substitution. RF and AX41 dedicated signaling logs contain only epoch time, response/upstream status and duration; they exclude request URI/query, addresses, headers, cookies, credentials and identifiers.

Before canary enablement, run `scripts/collect-rf-media-relay-window.sh --self-test` with the workstation's `/usr/bin/awk`, then require a non-empty timed RF CSV and summary plus authenticated `scripts/collect-production-media-window.sh` aggregates for the identical UTC interval. Copy only the two dedicated privacy-safe signaling logs to protected temporary files and aggregate the exact epoch interval with `scripts/collect-classroom-signaling-nginx-window.sh --rf-log RF_LOG --ax41-log AX41_LOG --start START_EPOCH --end END_EPOCH`; delete the temporary copies after recording aggregates. Pipe only already time-bounded LiveKit logs into `scripts/collect-livekit-signaling-window.sh --signaling-contour rf-two-hop`; that collector emits event counts and never raw lines. Missing sources, empty input, parse/tunnel/exporter errors, or unavailable authenticated metrics block enablement. Classify RF/AX41 signaling, LiveKit continuity, TURN/media, shared-host pressure, Selectel-to-AX41 reachability, application/collaboration WebSockets, and client resources separately; store no URI/query, address, cookie/authorization, credential/token, SDP/candidate, or participant/room/lesson identifier.

In a proven zero-participant preflight window, the two signaling-route counters, two media-policy counters, and RTT/jitter/loss histograms may legitimately have no event series yet. The production collector emits bounded zeroes for only those seven aggregates when the independently scraped `livekit_participant_total` is present and exactly zero. It must still fail closed if that participant source is absent, if participants are nonzero while an aggregate is missing, or if the core target gate is not `1`; never replace the query with an unconditional `or vector(0)` fallback.

Only after those gates may a separately reviewed production config release change `livekit.regionalRelay.mediaMode` to `rf-turn-relay`; `signalingMode` remains `rf-two-hop`. First compare the API Secret and RF auth file only in process memory and emit only `match|mismatch`. Then a protected verifier must obtain a fresh authorized exact Origin `https://online.honeyschool.ru` room token, validate the response shape without printing the token or credential, and use that exact credential for a real RF allocation and bidirectional-data check. Direct `.school` must still return `wss://online.honey.school/livekit` without regional media. Repeat a two-browser `.ru` smoke without VPN with camera, microphone, bidirectional subscriptions, and one screen-share cycle. On each classroom shell require `data-media-all-relayed="true"`, a positive `data-media-peer-connections`, and `data-media-transport-class=turn-udp|turn-tcp|turn-tls`; these values are derived in memory from the selected candidate pair and never retain addresses, URLs, candidate ids, SDP, room, or participant data. Correlate them with nonzero coturn `active_turn_allocations`. Signaling logs, RF bandwidth, a policy response alone, or a generic relay candidate are not media proof. Any `server lost`, persistent `checking`, signal-source close, unexpected duplicate identity, signaling/media reconnect, missing RF allocation, or threshold breach triggers the reviewed media-only `off` release; allow allocations to drain and verify `.school`, RF web/API/signaling, nginx, LiveKit, identity, attendance, and classroom data.

Monitor the complete 45-minute lesson and stabilization interval through an SSH tunnel to loopback node-exporter plus authenticated production VictoriaMetrics. Pass requires zero media reconnects, packet loss below `2%`, RTT p95 below `300 ms`, jitter p95 below `50 ms`, total edge CPU below `70%`, at least `512 MiB` available memory, RX and TX each below `30 Mbps`, healthy nginx/coturn, and zero swap, OOM, UDP-buffer errors, nginx/coturn restarts, or allocation failures. Correlate participant continuity, NACK, pod restarts, RF/API/WebSocket health, and relay values for the exact window. Report chat-WebSocket reconnects separately: a media pass does not close that defect. Evidence contains only aggregate timestamps and values.

Run `scripts/collect-production-media-window.sh` on the production guest against `http://127.0.0.1:32085/victoria-metrics`, passing the exact UTC start/end as epoch seconds. It returns only aggregate CSV values for joins/continuity, RTT, jitter, loss, NACK, restarts, UDP errors, core targets, RF blackbox probes, and collaboration health. The current production metrics do not expose chat-WebSocket reconnect count, so the collector emits `not_available`; record the browser observation separately and do not infer zero. Correlate that CSV with the RF-edge collector by the same timestamps, never by participant or room labels.

Rollback sets only `livekit.regionalRelay.mediaMode=off` through the reviewed production GitOps path, affecting new room-token media responses while preserving healthy `signalingMode=rf-two-hop`. Do not terminate active allocations, restart nginx or LiveKit, mutate lesson/collaboration data, alter DNS, or touch a legacy host. After allocations reach zero, coturn alone may be stopped; verify nginx and the `.ru` web/API/WebSocket signaling paths remain healthy. Certificate renewal only writes `/run/playsay-rf-edge-media-relay-certificate-reload-required`; consume it after zero allocations, then repeat TLS allocation. Sustained media routing requires authenticated continuous monitoring. A dedicated/second relay, resize, wider range, or more concurrency requires a separate reviewed provider-state/capacity change.

#### Lesson admission and expired media-credential recovery

Diagnose shared-link admission before room-token or media. For a bounded UTC/MSK window, compare only aggregate `LINK_STARTED`, `REMEMBERED_SESSION_REJECTED`, email challenge, Lobby, assertion and `CONFIRMATION_ABANDONED` outcomes. An accepted link with no later proof stage is an unfinished identity-confirmation handoff; it is not a room-token, LiveKit or TURN failure. Retain counts and coarse lesson state only. Do not copy subjects, emails, link aliases/capabilities, attempt secrets, authorization URLs, room tokens, addresses, raw authentication errors or lesson content into logs, evidence or chat.

For an established classroom that loses media, align the same bounded interval across browser lifecycle, privacy-safe signaling counts, LiveKit resume/connection aggregates and coturn authentication aggregates. Signaling reachability does not prove RTC recovery. If a page reload obtains a fresh authorized room token and restores bidirectional media while the mounted session repeatedly retries with expired regional credentials, use reload as the immediate participant workaround. Do not restart coturn, LiveKit, nginx or k3s, rotate secrets, change DNS/firewall, or interrupt other active lessons for this application-session defect.

Deliver the permanent application correction non-disruptively through the normal numeric-release GitOps path, rebuilding only affected web and API Gateway images. Before promotion require a zero-active-lesson check and reviewed immutable digests. Do not include coturn/LiveKit/edge configuration changes unless separate evidence identifies an independent infrastructure defect. After sync, record ArgoCD readiness separately from user acceptance.

Accept lesson entry first in fresh desktop and phone browser contexts: the shared link must be visibly accepted, verified-email and teacher-Lobby confirmation must settle, and classroom pre-join may appear only after identity proof and assertion handoff. Then accept media recovery in a separate two-participant `.ru` lesson: keep the established connection beyond credential expiry, interrupt only learner media, require one fresh-token recovery, one current participant per identity, renewed TURN allocation and restored inbound/outbound media. Store only expiry class, bounded outcome/timing and coarse `TURN_UDP|TURN_TCP|TURN_TLS` transport.

If either path regresses, return only web and API Gateway to their previous reviewed immutable digests through GitOps and repeat the corresponding acceptance. During rollback the participant reload workaround remains valid. Preserve lesson/admission/collaboration data and the healthy signaling route; do not substitute a coturn/LiveKit restart or secret/network change for application rollback.
...
Production hotfix `release/01.005.03` restores compact reusable lesson links on both `online.honeyschool.ru` and `online.honey.school` and preserves the originating application domain through login and password recovery. Platform `develop` and the numeric release both use source commit `2a3987a6556fb56cbc13fbaa7a691c8fdbc0329b`; immutable API and web digests are recorded in the ready candidate. A fresh four-database logical backup passed SHA-256 and archive-list validation, and the bounded production Liquibase Job applied exactly the reviewed additive compact-link changeset before rollout with zero pending changesets afterward. All 19 ArgoCD applications reached `Synced/Healthy` at infra candidate `0da1a441d63af3c4b2913d253c2bb8ee1c4f2e59`. Browser acceptance on both origins confirmed public compact resolution, fragment removal, origin-specific OIDC callbacks and keyboard-accessible forgot-password continuity. Follow-up RF-edge release `release/01.005.04` excludes API JSON from Selectel origin substitution, so `.ru` responses retain the explicit `.school` copy choice; its check/apply passed nginx validation and the repeated disposable production lesson journey passed compact/legacy entry, both origins, Lobby, remembered entry, kick/readmit, rotation/revocation and cleanup. Bounded reset canaries for the assigned teacher and reported student each reached `SENT / MAILJET_API / DELIVERED / sent`; inbox placement and completion remain owner-confirmed. The disclosed incident link was rotated from inactive revision 1 to active revision 2 through the authorized application endpoint without replaying or retaining the old token. Evidence: `migrations/ax41/evidence/20260830-release-01.005.03.md`. Roll back the production root to `release/01.005.02` for the application hotfix and reapply RF-edge release `release/01.005.03` for the edge-only change; retain the additive nullable columns.
...
For a reviewed route/log-only reconciliation that must preserve an unrelated pending landing-page or asset update, use the narrower `--tags edge-proxy-routes` from the exact numeric release. Run `--check --diff` first and require that only the declared nginx route/TLS file changes; then apply and repeat check with `changed=0`. This tag still validates nginx before and after the reload and never bypasses the Ansible source of truth.

For the classroom route probes and privacy-safe correlation rollout, use `--tags classroom-route-observability` on the AX41 host playbook and `scripts/apply-rf-edge-release.sh --check|--apply --route-observability-only` on Selectel. This scope excludes landing pages, firewall, coturn and the node-exporter service configuration; it may render the two managed nginx route files, activate the RF probe timer and gracefully reload nginx after validation. Require a zero-participant window, review both diffs, record service restart counters before and after, and require a second check with `changed=0`.

For the HTML-game upload emergency, use `--tags html-game-upload-routes` on the AX41 host playbook and `scripts/apply-rf-edge-release.sh --check|--apply --html-game-upload-only` for Selectel. These source-controlled modes change only the production `online` route file, validate nginx, and activate it through a graceful reload; they do not render landing pages, touch firewall/packages, or run coturn. Their check-mode diff must contain only the bounded two-path `21m` location block. A later full route reconciliation replaces the managed emergency block with the equivalent location already present in the complete templates.
...
Инцидент release `01.005.00` показал, что совпадение source commit у release/develop не гарантирует применённую schema: production vocabulary pod запускал новый код поверх ledger только до 2026-07-29, и `/api/vocabulary/overview` отвечал `500` из-за отсутствующих таблиц/колонок. Ремедиация, backup и точный applied set зафиксированы в `migrations/ax41/evidence/20260828-release-01.005.00-vocabulary-schema-remediation.md`; последующие promotion обязаны проходить общий schema-convergence gate выше.
...
Production `release/01.006.07` enables environment-local Web Push and unread-based PT2M/PT30M email timing with the mixed-role chat and tools-rail hotfix. Operational rollout checks passed on both public origins; owner-assisted browser/inbox acceptance remains open. In particular, the dev Mailjet timing canary was accepted but temporarily deferred by the provider, so local `SENT` must not be presented as mailbox delivery. Delivery evidence and rollback to `release/01.006.06` are recorded in `migrations/ax41/evidence/20260903-release-01.006.07-chat.md`.
...
Browser chat notifications are an explicit opt-in Web Push path independent of email eligibility. The chart keeps `chatPush.enabled=false` by default. Each environment may enable it only after the P-256 private key and a `mailto:` or HTTPS subject exist as keys `private-key` and `subject` in the environment-local `playsay-chat-push` Secret. Run the reviewed `scripts/provision-chat-push-secret.py --namespace playsay-dev --subject https://dev.online.honey.school` on the dev guest, or use `playsay-prod` and `https://online.honey.school` on the production guest, with the matching privileged kubeconfig. It requires Python 3 with cryptography and kubectl, preserves an existing key, fails closed on lookup/incomplete-secret errors, and outputs only the matching public key. Put that public key in the matching environment values and enable push through GitOps. Never print, commit, reuse across environments, or expose the private key through capability responses. A VAPID key rotation requires a separately approved replacement of Secret and public value together, rolling out `api-gateway`, and asking users to disable and enable notifications again because existing browser subscriptions are bound to the old application-server key.
...
4. Send more messages after the first email. The next due time is `max(first new message + 2 minutes, last successful sent_at + 30 minutes)`; a healthy scheduler sends within the following 30-second poll. Without new messages there must be no repeat at all. Existing pending batches respect the new cooldown; historical messages are not backfilled.
5. Repeat with the recipient reading before `due_at`: the digest becomes `SKIPPED`. Merely returning online or keeping a hidden tab/socket connected does not suppress unread email. A recipient without email is also `SKIPPED`; provider errors retry with the configured 1/5/15-minute backoff and eventually become `FAILED` without losing chat messages. Message persistence and digest enqueue are one transaction, and digest dispatch transactions are isolated per recipient.
6. With notifications disabled in chat, confirm the browser does not request permission. Enable them from the chat bell and confirm the authenticated `GET /api/chat/push/capability` returns `available` and the public key, followed by one active subscription for that account. Permission denial must leave chat and email usable and show the localized denied state with browser-settings guidance. Missing configuration shows unavailable with retry, never a silently inert action. Repeat acceptance on both `online.honey.school` and `online.honeyschool.ru`; browser permission and subscriptions are origin-specific.
...
```bash
curl -fsS -H "Authorization: Bearer $ACCESS_TOKEN" https://dev.online.honey.school/api/chat/push/capability | jq '{available, publicKeyPresent: (.publicKey | length > 0)}'
...
The required unpacked dev extension is version `0.1.8` or later. It allows `dev.online.honey.school`, `online.honey.school`, and `online.honeyschool.ru`, includes its package version in the version-1 `AWAITING_ACTION` acknowledgement, and temporarily uses the Chrome Debugger permission during an active activity. Version 0.1.8 adds per-event results, extension-side deduplication, and viewport refresh after resize or navigation. Older packages are not acceptance eligible; the web must show `EXTENSION_UPDATE_REQUIRED` before capture. Regenerate the ZIP and installation guide and confirm `0.1.8` on `chrome://extensions` before distribution. Every later shipped extension source, manifest behavior/permission, runtime bundle, or user-facing asset change requires another patch increment.

Before accepting a dev build, run the focused web/extension tests, localization integrity suite, production-mode build, and browser matrix on `https://dev.online.honey.school/` with one teacher and at least one student: student-first launch before teacher connection, outdated-extension rejection, successful acknowledgement/capture, real provider input from both roles, missing or disabled extension, closed provider tab, Retry with a new session, Return to lesson, and an immediate stop/relaunch. An authoritative teacher stop must clear an unowned student `REQUESTED` state even when its session id differs; an old host stop must not erase a replacement host session. A matching named video track from the trusted teacher must recover `ACTIVE` if a lifecycle packet is missed, and the student must show a localized connecting state until the track attaches. Participant pointer, keyboard and coalesced wheel input must use the dedicated `playsay-external-activity-v1` collaboration channel, independently of HTML-game realtime mode. If the socket is unavailable or its acknowledgement is uncertain, the same event id falls back through LiveKit and the host deduplicates it before extension dispatch. Verify the returned stable result and the input-only degraded UI without tearing down capture. The student must return to ordinary material without a black video surface and follow the replacement session.

The chart exposes this channel separately as `collaboration.externalActivityRealtime.enabled`; `required=true` fails rendering when it is disabled. Dev and production enable and require it, while `GAME_REALTIME_MODE` remains independently configurable and may stay `off`. Before delivery, render both environments and run:

```bash
./scripts/validate-external-activity-realtime.sh
```

During an authorized incident check, correlate only bounded counters and sanitized failures by timestamp, stage, transport, and stable result code. Useful metric families are `playsay_external_activity_active_connections`, `playsay_external_activity_buffered_amount_bytes`, `playsay_external_activity_input_total`, `playsay_external_activity_input_result_total`, `playsay_external_activity_input_latency_seconds`, and `playsay_external_activity_message_bytes`. Failure logs use the fixed event name `external_activity_input_failure`; do not query or export payloads. Successful input is aggregate-only. Never retain provider URL/content, pointer coordinates, key values, cookies, credentials, authorization data, tokens, or learner data.
...
After separate promotion authorization and the normal backup/migration gate, promote only that ready GitOps candidate. Canary extension 0.1.8 on both `https://online.honeyschool.ru/` and `https://online.honey.school/`: verify versioned acknowledgement, capture, a responsive Wordwall action from teacher and unlocked student input with returned results, one actionable negative path, Retry with a new session, Return to lesson, and immediate relaunch without a black participant surface. Stop promotion acceptance if either origin fails.

Collect only extension version, web build identity, timestamps, lifecycle/input stage, transport, and stable lifecycle/result codes. Never record provider page content, raw Chrome errors, target tab/stream ids, participant input values or coordinates, cookies, credentials, authorization data, tokens, or learner data.

Fast-lane rollback is GitOps-only: set `collaboration.externalActivityRealtime.enabled=false` and `required=false`, render/validate the chart, and promote the reviewed configuration. Existing clients then use the same-event-id LiveKit fallback; verify teacher and student input before declaring rollback complete. If the web itself must roll back, restore the previous immutable web image or produce an explicitly disabled web build through the reviewed GitOps path, then verify the normal classroom on the affected origin. Do not roll back backend/data, change extension permissions, use direct `kubectl apply`, or contact the deleted legacy contour. Extension 0.1.8 remains compatible with the disabled web release.
...
The AX41 CI rollout uses a closed OWASP 13.0.0 database snapshot at `dependency-security-seed` inside each existing JVM `jenkins-agent-cache` Gradle subdirectory (including `gradle-full` and `gradle-keycloak-lesson-authenticator`). Provision the identical checksum-verified snapshot with read-only file permissions before running `scripts/configure-jenkins-dependency-security-cache.groovy` through the authenticated Jenkins Script Console. The script preserves other global environment entries and sets only `DEPENDENCY_SECURITY_CACHE_SEED=/home/gradle/.gradle/dependency-security-seed`; no controller restart is needed. Never seed from an active database, include credentials, or replace a snapshot while a job copies it. Each invocation copies the snapshot into its private workspace and must refresh advisories successfully. Dependency-Check KEV analysis remains enabled; the platform runner defaults to CISA's official `cisagov/kev-data` GitHub mirror because the canonical `www.cisa.gov` JSON endpoint may return 403 from CI egress networks. Override `DEPENDENCY_SECURITY_KEV_URL` only to an approved CISA mirror and record the override in release evidence. Periodically replace the seed between builds using a completed current-schema run; a missing, incompatible or unrefreshable seed fails the gate. To return to cold bootstrap, remove only this global environment entry.
...
The current agent SSH route is the explicit key and direct public AX41 jump host shown above; it never requires WireGuard. Use destination `playsay@10.60.0.20` for production Kubernetes diagnostics, `playsay@10.60.0.30` for dev, and `playsay@10.60.0.40` for CI/Jenkins. The old `146.103.126.15` controller is rollback-only and no longer receives GitHub webhooks; `89.124.113.223` is retired. Do not connect to either old address without an explicit owner request naming the legacy contour. Unauthenticated Jenkins API calls return a login redirect or `Authentication required`; that only means auth is missing, not that the job is down. For POST requests such as job reconfiguration or manual `buildWithParameters`, also request a crumb from `/crumbIssuer/api/json` and send the returned cookie plus crumb header. When a dispatcher build has several downstream results and only one module failed, retry that module job directly with the original `BRANCH_NAME`, `GITHUB_BEFORE`, and `GITHUB_AFTER` instead of rebuilding successful modules. Keep Jenkins passwords, crumbs, GitHub tokens, and kubeconfigs out of logs and chat.
...
Deletion is asynchronous and idempotent. `DELETE /api/admin/user-management/users/{subject}` returns `202` with an operation id; read `/api/admin/user-management/operations/{operationId}` until `COMPLETED` or `FAILED`. ADMIN and ADMIN+TEACHER have identical rights to delete other account types. Deletion does not require a replacement teacher; the legacy optional parameter is ignored for new operations. Self-admin deletion and removal of the last available administrator are refused. An `IN_PROGRESS` lesson involving the target as teacher or student blocks acceptance without terminating the lesson.

The checkpoint migration `2026-09-06-001-user-deletion-checkpoints.xml` adds `user_deletion_operation.stage` with `LEGACY` for existing rows. New operations persist `REQUESTED → SUSPENDED → LOCAL_CLEANED → DATA_PURGED → IDENTITY_DELETED → COMPLETED`. Deliver the registration-service suspension endpoint before gateway migration/application and web-app through normal CI/GitOps gates; do not run the migration manually. Accepted intent blocks old JWT access immediately. Suspension disables the exact Keycloak identity and logs out its sessions before cleanup. Cleanup detaches links without deleting other accounts, then performs downstream purge, Keycloak deletion and final anonymization in separate checkpoint transactions.

Recovery scans stale PENDING/RUNNING operations every 30 seconds. FAILED operations retain their checkpoint and local access block; after diagnosing the failed dependency, a newly confirmed DELETE resumes the same operation id. Reading status never resumes or retries deletion. Do not manually enable the identity after partial cleanup. A missing identity is accepted when replaying disable/logout/delete. Browser request timeouts (30 seconds per request, 60 seconds per polling attempt) do not cancel server work; use the known operation id to check the outcome and never infer completion from 202.

Before rollback to a gateway version without checkpoint support, verify there are no non-LEGACY operations whose status differs from COMPLETED, including FAILED. Such operations must be resolved with the checkpoint-aware version; old workers could replay incompatible cleanup and old JWT filters do not enforce the intent block. Keep the additive stage column during application rollback. Application rollback does not restore deleted personal data or Keycloak identities. Any recovery of data requires the separately approved backup/restore procedure, not automatic re-enablement.
...
Deliver the vocabulary purge fix before enabling checkpoint-aware administrative deletion: the old creator-based practice deletion cascades into other learners' results. The new purge preserves shared practices and removes only the exact target's owner scope from plans/settings. Drain existing PENDING/RUNNING deletions before replacing old workers; do not allow mixed old/new deletion processors to handle the same operation during rollout.
...
The same service exposes deterministic `POST /internal/html-game-optimizations` for upload-time embedded PNG/JPEG/WebP reduction. It uses the same internal token but no OpenAI or Chromium call, accepts a 24-MiB JSON envelope, enforces a 30-second deadline and retains the adaptation route's 7-MiB limit. Gateway invokes it only after the raw 20-MiB and HTML checks, validates `embedded-raster-v1` aggregate accounting, revalidates returned HTML and stores exactly one final asset. Existing assets are not backfilled. A required optimizer failure is fail-closed: `MATERIAL_HTML_GAME_IMAGE_INVALID` is terminal for that file and `MATERIAL_HTML_GAME_OPTIMIZATION_UNAVAILABLE` is retryable; neither creates partial material or lesson state.

For SDK adaptation the authenticated cluster-only route still receives the canonical game so the service can run full source/candidate Chromium comparison. Before any OpenAI request, `game-adapter-service` replaces threshold-triggering contiguous base64 image/audio data URIs with opaque in-memory placeholders (256 KiB individual or 1 MiB aggregate; no whole-game 3-MiB trigger). OpenAI receives only the reduced HTML. The service requires every issued placeholder exactly once and unchanged, restores canonical media byte-for-byte, then runs static, runtime and `mechanics-v3` validation on the complete game. The bounded manifest is never persisted, logged, returned or sent to OpenAI. `ADAPTED_HTML_MEDIA_INTEGRITY_INVALID` / public `GAME_ADAPTER_MEDIA_INTEGRITY_INVALID` is terminal and creates no adapted asset or material mutation.
...
For optimizer diagnostics, use a synthetic non-sensitive fixture through the normal authenticated upload API; do not send uploaded HTML or the service token directly from the shell. Confirm asset metadata reports a bounded `imageOptimizationStatus`, policy, byte counts and image counts, then inspect only aggregate metrics and safe log fields:

```bash
kubectl -n playsay-dev logs deploy/game-adapter-service --since=15m | grep -E 'html_game_image_optimization'
kubectl -n playsay-dev logs deploy/api-gateway --since=15m | grep -E 'HTML game image optimization'
kubectl -n playsay-dev logs deploy/game-adapter-service --since=15m | grep -E 'game_adaptation_media_protection'
kubectl -n playsay-dev describe pod -l app.kubernetes.io/name=game-adapter-service | grep -E 'OOMKilled|Evicted|runtime-tmp'
```

Never add `-x`, print pod environment variables, response bodies, data URIs, prompts, placeholder tokens, digests or Secret contents while diagnosing these routes. Media-protection logs are accepted only when they contain status, canonical/reduced byte counts, extracted bytes, resource count and a safe failure code. Health proves only process readiness; dev acceptance requires a synthetic large-image/audio adaptation proving the provider request has no extracted payload, followed by material-preview and classroom rendering of the restored result. Roll back through GitOps by restoring both prior gateway and game-adapter image digests together. Already optimized self-contained assets remain valid and require no data rollback; disabling media protection reverts only future adaptation requests and does not require a manifest migration because manifests are never persisted.
...
Select checks and reuse evidence according to [verification efficiency](verification-efficiency.md). Preserve failed runs and mandatory acceptance gates; repeat checks affected by changes, drift or new symptoms.

The web application exposes its local connection panel with Ctrl+Alt+Shift+D on Windows/Linux or Cmd+Option+Shift+D on macOS, and a diagnostics button. Escape or the same shortcut closes it. The physical D key is layout-independent; editable controls and composition/repeated key events are excluded. The panel separates expected topology from observed public endpoints and selected TURN relay evidence, expires observations after 15 seconds, and remains readable when telemetry intake fails. An endpoint response alone does not certify the internal network path. Use separate publisher/subscriber media evidence and received-media events when assessing a lesson. UI availability in a source candidate is not production delivery evidence. Dev delivery and the separate failed/successful long-run evidence are recorded in [RF dev connection diagnostics acceptance](../migrations/ax41/evidence/20260905-rf-dev-connection-diagnostics.md); retain both outcomes when assessing a production candidate.

Before promotion, run `python3 scripts/audit-rf-turn-parity.py --ssh-key /protected/key/path` from the reviewed infra revision. It checks actual dev/prod TURN options, intentionally distinct port ranges, independent nonempty REST secrets and certificate references, and emits booleans only. A passing configuration gate still requires browser allocations, received media and the production-specific network canary. It never changes either service.


### Isolated AX41 dev GeoIP delivery

Use `scripts/apply-ax41-geoip-dev.sh <full-clean-pushed-infra-commit> syntax|check|apply`. This play installs the local reader/updater and a shared classification map, but only inserts navigation hooks into the four existing dev web/key HTTP/HTTPS vhosts. It refuses an enabled production flag and preserves all other vhost bytes. Package installation suppresses automatic service starts/restarts; nginx receives only a validated graceful reload. The full edge role consumes the same separately managed policy file.

Initial download is explicit: run `systemctl start honey-school-geoip-update.service` on AX41 after the disabled-safe play. Check `mmdblookup` for country_code and asn on public IPv4/IPv6 fixtures, the protected state metrics and timer. Never print provider credentials. Daily activation is serialized and restores the previous database/configuration on validation or reload failure without advancing the success timestamp.

Enable or roll back dev entry by changing only `honey_school_geoip_redirect_dev_enabled` in Git and repeating the exact-ref play. The original dev-only play assumes production is disabled; after production activation, reconcile flags through the scoped production policy play below. Set `honey_school_geoip_updates_enabled: false` in Git and apply to pause the timer independently; preserve the last verified database and its age limit. Restore the validated last-good database with `/usr/local/sbin/honey-school-update-ipinfo-lite --rollback-last-good` after pausing the timer; its original success timestamp is restored, not renewed; never edit credentials to simulate provider failure.

### Scoped production GeoIP activation and rollback

Production browser redirects are enabled for honey.school, online.honey.school and key.honey.school; dev retains its independent enabled flag. Apply `scripts/apply-ax41-geoip-prod.sh <full-published-commit> syntax|check|apply` from a clean checkout whose current branch HEAD is published. This changes only the shared policy and six production HTTP/HTTPS root-location includes, validates nginx and performs a graceful reload. Existing database, updater, credentials, other vhosts and application VMs are not provisioned by this play. Failed validation/activation restores both previous files. Verify the actual MMDB age/country classification, flags, six includes, exclusion matrix and unchanged nginx master PID. Synthetic loopback tests prove policy, not a real RF last-mile browser session.

Rollback sets only `honey_school_geoip_redirect_prod_enabled: false` in Git and repeats the same exact-ref workflow. The existing dev provisioning play still refuses an enabled production flag; use this scoped policy play for flag reconciliation once production is active, and review separate provisioning changes before touching the shared host. Keep API, auth/callback, WebSocket, LiveKit, collaboration, ACME, ops and trusted Selectel-peer exclusions. Unknown/non-RU clients do not redirect and .ru never redirects back.
