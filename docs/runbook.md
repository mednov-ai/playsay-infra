...
AX41 at `65.109.55.110` serves `honey.school` production from `release/01.004.12`: the Ubuntu 24.04 physical host, healthy mdadm RAID1/ext4, KVM/QEMU/libvirt, OpenTofu, firewall, WireGuard and VPN-only Cockpit baseline are active. The NAT-backed `playsay-prod` (`10.60.0.20`, 8 vCPU/38 GiB), `playsay-dev` (`10.60.0.30`, 2 vCPU/10 GiB) and `playsay-ci` (`10.60.0.40`, 4 vCPU/8 GiB) guests are active and protected by separate OpenTofu states. The 14 declared guest vCPUs moderately overcommit the host's 12 logical CPUs, while fixed guest memory remains 56 GiB and leaves about 6 GiB for the host. Prod/dev run independent k3s, ArgoCD and Sealed Secrets controllers. CI runs only k3s, Sealed Secrets and the Git/JCasC-defined Jenkins controller; it has no product workloads, ArgoCD, Keycloak or MinIO. Production has 19 ArgoCD applications; all are `Synced/Healthy` at infra release commit `0182f4620bfa3c79defb48e99eda65575ab9966b`. Web, keyboard, API, collaboration signaling, LiveKit signaling, forced TURN, transactional Mailjet email and production monitoring reach AX41. Canonical Keycloak is `ops.*/keycloak`; the explicit `key.*/keycloak` compatibility path remains only until owner-operated login/material acceptance. Both GitHub webhooks use `hooks.honey.school`. Payment remains disabled until independent production credentials are supplied. Object Storage remains a post-stabilization state/backup task, not a blocker to this cutover. Release evidence: `migrations/ax41/evidence/20260826-release-01.004.12.md`; prior capacity evidence remains `migrations/ax41/evidence/20260727-release-1.001.08-capacity.md`.
...
Lesson participant links use two fail-closed api-gateway origins: `PLAYSAY_PUBLIC_APP_URL` is `https://online.honey.school` in production, while `PLAYSAY_PUBLIC_APP_RF_URL` is `https://online.honeyschool.ru`; both values are `https://dev.online.honey.school` in dev. The API accepts only the declared link-origin enum and applies the selected origin to both the generated link and managed-student `continueUrl`. Do not replace either setting with a client-supplied URL or add a legacy application origin.
...
## Shared External Activity Dev Acceptance

The web pipeline exports `VITE_EXTERNAL_ACTIVITY_ENABLED=true` only for dev builds. `scripts/ci/validate-ci-contracts.sh` verifies that the dev branch of `Jenkinsfile.web-app` sets the flag and that numeric production release builds leave it unset. Shared external activities are not production-eligible while the browser lifecycle matrix remains incomplete.
...
Dev rollback is web-only: restore the previous dev web image or produce an explicitly disabled dev build through the reviewed GitOps path, then verify the normal classroom. Do not roll back backend/data, change extension permissions, use direct `kubectl apply`, or modify the protected legacy contour. Production build, release-candidate creation, promotion, and rollback are not authorized by this dev-only work.
