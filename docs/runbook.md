...
The 2026-09-01 no-VPN canary failed as `SIGNALING_ROUTE_UNAVAILABLE`: RF TURN/UDP was selected, while room tokens still named direct `.school` signaling; LiveKit recorded signal-source closures and replacement loops, Selectel remained healthy, and VPN restored both clients. Keep `livekit.regionalRelay.mode=off` as the safe baseline for every support release. Evidence is in `migrations/ax41/evidence/20260901-rf-classroom-signaling-canary-failure.md`.

Application support is deployed first with `livekit.regionalRelay.mode=off`. Before any edge apply, run `scripts/validate-rf-classroom-signaling-route.sh --static`; after the reviewed RF release, run its `--probe` mode and require bounded unauthenticated responses from both `.ru` two-hop and `.school` direct paths. Then run RF Ansible check/apply during a zero-active-lesson window and repeat check with `changed=0`; nginx and coturn must remain healthy. The explicit RF `/livekit` locations preserve the path and WebSocket Upgrade over verified TLS 1.2 to `online.honey.school`, disable buffering and never perform response substitution. RF and AX41 dedicated signaling logs contain only epoch time, response/upstream status and duration; they exclude request URI/query, addresses, headers, cookies, credentials and identifiers.
...
In a proven zero-participant preflight window, route counters and RTT/jitter/loss histograms may legitimately have no event series yet. The production collector emits bounded zeroes for only those five aggregates when the independently scraped `livekit_participant_total` is present and exactly zero. It must still fail closed if that participant source is absent, if participants are nonzero while an aggregate is missing, or if the core target gate is not `1`; never replace the query with an unconditional `or vector(0)` fallback.

Only after those gates may a separately reviewed production config release change the single policy to `rf-origin-relay`. A fresh authorized exact Origin `https://online.honeyschool.ru` room token must atomically contain `wss://online.honeyschool.ru/livekit` plus relay-only RF media; any mixed response fails closed. Direct `.school` must still return `wss://online.honey.school/livekit` without regional media. Repeat a two-browser `.ru` smoke without VPN with camera, microphone, bidirectional subscriptions, and one screen-share cycle before the real lesson. Any `server lost`, persistent `checking`, signal-source close, unexpected duplicate identity, signaling/media reconnect, or threshold breach triggers the reviewed flag-first `off` release; allow allocations to drain and verify `.school`, RF web/API, nginx, LiveKit, identity, attendance, and classroom data.
...
Rollback sets `livekit.regionalRelay.mode=off` through the reviewed production GitOps path, affecting only new room-token responses. Do not terminate active allocations, restart nginx or LiveKit, mutate lesson/collaboration data, alter DNS, or touch a legacy host. After allocations reach zero, coturn alone may be stopped; verify nginx and the `.ru` web/API/WebSocket paths remain healthy. Certificate renewal only writes `/run/playsay-rf-edge-media-relay-certificate-reload-required`; consume it after zero allocations, then repeat TLS allocation. Sustained routing requires authenticated continuous monitoring. A dedicated/second relay, resize, wider range, or more concurrency requires a separate reviewed provider-state/capacity change.
