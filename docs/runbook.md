...
Trigger `codex/*`, `feature/*`, and `hotfix/*` branches manually through the dispatcher with an explicit `BRANCH_NAME` and `FORCE_TARGETS` when a dev deploy of that branch is needed. Keep `MAX_PARALLEL_MODULE_JOBS=1` for normal dev runs and during any CPU/RAM pressure. A manual override to `2` is allowed only after checking that `MemAvailable >= 15%`, swap is not growing, and there are no fresh pod OOM/restarts. Trigger a module job directly only for recovery/debugging.
