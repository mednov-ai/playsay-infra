...
The `playsay-web-app-develop` Node container sets `NODE_OPTIONS=--max-old-space-size=768` inside its `1Gi` memory limit. The Vite production bundle exceeds Node's cgroup-derived ~512Mi default heap; keep the explicit heap below the pod limit to leave room for native/transformation overhead.
