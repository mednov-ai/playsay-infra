...
Branch routing is strict. `playsay-platform-dispatch-webhook` is the only job that owns the Generic Webhook Trigger token; it validates each push payload and forwards it to exactly one internal dispatcher. `playsay-platform-dispatch-develop` accepts only `develop` and aborts its older dispatcher when a newer develop push arrives. `playsay-platform-dispatch-release` accepts only a protected numeric three-part branch named `release/<version>.<subversion>.<patch>`, for example `release/1.001.07`, and serializes release candidates independently of develop. Both internal dispatchers share the global four-agent CI cap. `codex/*`, `feature/*` and `hotfix/*` publish to dev only through direct manual starts of the required module jobs with explicit branch/commit parameters; they have no webhook dispatcher. Tag/delete events are ignored and free-form `release/*` names fail before routing.
...
After every affected module succeeds, the release finalizer verifies the source HEAD, affected `build.commit` and digests, unchanged image/build metadata for unaffected charts, every prod `targetRevision`, and rendered `repository@sha256` references. A clean finalizer agent registers only the allowlisted dependency repositories declared by production charts (currently Bitnami for Keycloak) before `helm dependency build`; any unknown remote repository fails closed and must be reviewed before it is added. Only then does the finalizer change the schema-v2 manifest to `status: ready`; any failure leaves `building`. Jenkins has no production kubeconfig and cannot sync the prod cluster. Starting with `release/1.001.08`, only the reviewed Argo Workflows promotion described below may move a ready candidate into production. Never deploy prod directly from `main`, `develop`, a free-form release name or `hotfix/*`; publish a hotfix through a new numeric release branch.
...
`argocd-apps/prod/promotion-history/1-001-07.yaml`; do not fabricate a schema-v2
candidate for it. The first release that Argo Workflows may promote is
`release/1.001.08`. A legacy release may be a baseline or rollback target only when it
is named by `current-release.txt` or has an accepted promotion-history record.

The production control plane is intentionally not installed until every activation
...
3. A dedicated GitHub App has only the repository permissions required to read
   platform/infra state, create the one-file `current-release.txt` PR and enable its
   auto-merge after required checks. Branch protection and auto-merge must already be
   enabled.
4. The offsite S3-compatible backup bucket has versioning and Object Lock enabled.
   An owner has decrypted a test backup, verified checksums and recorded a successful
   restore drill.
5. Keycloak contains the confidential `playsay-release-workflows` OIDC client, the
   `release-operators` group and at least one explicitly named operator in that group.

The pinned deployment is Argo Workflows chart `1.0.23` (application `v4.0.8`, chart
SHA-256 `a20365b94f3c286eed01c1ca7bd1ec428efa002f5610f140dd4c933322d6bc6d`).
Controllers/UI live in `argo-workflows-system`; WorkflowTemplates and executions live
...
```bash
./scripts/build-release-ops-image.sh
```

Create the runtime Secrets from an approved secret manager; never commit their
values. The exact names and keys are listed in
`argo-workflows/secrets/README.md`. Configure Keycloak with a deliberately supplied
operator identity:

```bash
KEYCLOAK_OPERATOR_USERNAME='<existing-operator>' \
  ./scripts/configure-keycloak-prod-workflows.sh
```

After provisioning backup storage, run a real owner-operated restore test outside
the cluster, then record only its non-secret evidence:

```bash
RESTORE_DRILL_ID='<evidence-id>' \
  BACKUP_BUCKET_VERSIONING=Enabled \
  BACKUP_BUCKET_OBJECT_LOCK=Enabled \
  ./scripts/record-release-backup-readiness.sh
```

Apply the edge route/certificate from the normal AX41 Ansible control node, then
install only after all gates are true:

```bash
RELEASE_OPS_IMAGE='ghcr.io/mednov-ai/playsay-release-ops@sha256:<digest>' \
  ./scripts/install-argo-workflows-prod.sh
./scripts/validate-argo-workflows-prod.sh
```

Before the first real promotion, submit `release/1.001.08` with `dryRun=true`. Dry-run
validates the numeric branch, schema v2, reviewed infra SHA, platform HEAD, immutable
digests and baseline, but performs no backup, migration, sync or Git write:

```bash
argo submit --from workflowtemplate/playsay-production-promotion \
  --namespace playsay-release-system \
  -p releaseBranch=release/1.001.08 \
  -p dryRun=true \
  --watch
```

For a real promotion use `dryRun=false`. Resume the first suspend node with parameter
`approvedBy=<operator>` only after candidate/review verification; resume the second
with the same parameter only after all four backup Jobs are verified. The workflow
runs only declared `migrationTargets`, switches/syncs production in dependency order,
waits for all 15 Applications, runs direct and RF smoke, and asks the GitHub App to
merge a PR changing only `current-release.txt`. It then records `promoted` plus audit
metadata. A failure after sync restores the previous accepted application revision
and repeats health/smoke; it never restores a database automatically. Migrations must
therefore follow expand/contract.

Rollback accepts only an entry from promotion history and reuses existing immutable
images:

```bash
argo submit --from workflowtemplate/playsay-production-rollback \
  --namespace playsay-release-system \
  -p targetRelease=release/1.001.07 \
  -p dryRun=false \
  --watch
```

Approve its suspend node only after confirming that any database changes remain
backward-compatible. A data restore is a separate reviewed operation with its own
plan. If Argo Workflows itself is unavailable, stop automated delivery, retain the
mutex lease evidence, and use the legacy manual revision switch only under incident
review; never update `current-release.txt` until health and both direct/RF smoke pass.
...
Before release module jobs, Jenkins creates or resumes the matching infra release branch and writes schema-v2 `argocd-apps/prod/release-candidate.yaml` with `status: building`. A new branch starts from current infra `develop`, overlays running production `image`/`build` metadata from `current-release.txt`, and rewrites all prod ArgoCD `targetRevision` values to the new branch. Retries retain all previously affected targets and declared `migrationTargets`. After every affected module succeeds, the finalizer verifies the platform branch HEAD, affected `build.commit` and immutable digests, unchanged image/build metadata for unaffected charts, every prod target revision, and `helm template` output before changing the manifest to `status: ready`. Jenkins never receives a prod kubeconfig, never changes `current-release.txt`, and never syncs production. Starting with `release/1.001.08`, a reviewed Argo Workflows run is the only normal production promotion path.
