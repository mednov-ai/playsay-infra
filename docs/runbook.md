...
Before rollout run `./scripts/test-keycloak-passkey-theme.sh`; it rejects the label prompt and verifies the iPhone/iPad user-gesture branch. After rollout verify the pod image/readiness, the discovery document, the realm WebAuthn fields through the Admin API, password login, the once-per-browser offer, first and additional AIA registration, in-app rename/delete, cancellation, password reset fallback, and that a redirect without a newly persisted credential is reported as failure. Use Chromium's virtual authenticator for repeatable add/login/rename/delete smoke. On real mobile Safari verify that **Add passkey → Keycloak continuation button → native Face ID/Touch ID/passcode sheet → profile with one extra key** completes without a silent bounce; repeat cancellation and confirm no key appears. Also accept one real Touch ID or Windows Hello registration/login. Production promotion remains blocked until these real-device checks pass.
