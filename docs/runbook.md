...
The RF relay frontend uses a custom Play&Say HTML5 player. `VIDEO_PLAYBACK_LOADING` is a neutral pending state, not an unavailable error. The relay `<video>` must keep `preload="none"` and must not attach the stream `src` until the learner presses Play; before that point there should be no `stream response` log lines. After Play, normal browser Range requests may be large and may buffer client-side. That is acceptable as long as the backend continues to stream with `StreamingResponseBody` and does not cache video bytes.
