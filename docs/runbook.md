...
Sprint 6 homework/progress smoke is still a dev smoke checklist, not a committed Jenkins stage. Run it after rollout with real Keycloak demo users and `https://online.play-and-say.ru`: create a temporary published private material, create a standalone group homework for `student-demo` + `student-demo-2`, create a single-student homework, verify teacher UI shows due date/instructions and `0/N scored` without an initial `10/10`, verify the single-student assignment has no group indicator, submit wrong answers as one student and correct answers as the other, verify teacher group progress uses score/errors rather than status labels, resubmit improved answers, then create homework from a completed lesson and confirm the completed live lesson is not joinable while the homework remains visible. If `GET /api/assignments` returns `MATERIAL_NOT_FOUND`, check for active homework rows whose material was archived during prior smoke cleanup; current `api-gateway` must skip those rows in list endpoints so one stale assignment cannot break the teacher/student homework panels. Detail endpoints for such assignments may still return `404`.
