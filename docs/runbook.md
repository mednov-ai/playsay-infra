...
The `release/1.001.04` frontend candidates remove render-blocking Google Fonts traffic from both public SPAs and bundle Manrope/Roboto Flex in their images. Production promotion is limited to `web-app` (`web-release-1.001.04-6`, digest `sha256:8a8eaf71c7fbca52553e39ce32572532213201b94fe6462afcf991af0ba9f71b`) and `keyboard-app` (`key-frontend-release-1.001.04-6`, digest `sha256:598b52ba9327142903dcfd44c49c801c2f076895f4d607805cc9c50f7d0734ca`); the other production applications remain pinned to `release/1.001.02`. Both candidates passed their Jenkins tests, dev rollout and browser smoke before production selection. Keep the AX41 edge on TLS 1.2 and verify that production HTML and loaded resources contain no `fonts.googleapis.com` or `fonts.gstatic.com` request before removing any temporary edge substitution.
...
Normal cleanup runs in Jenkins `post always`. The `playsay-ci-capacity-watchdog` CronJob runs every minute and restores saved replicas when the agent pod disappears or the 40-minute deadline expires. The `capacity-guard` sidecar deletes the agent pod immediately on NodeNotReady or after five 15-second samples (a full minute between the first and last sample) with `MemAvailable <1GiB` or I/O wait `>30%`; watchdog then restores services. A cluster-wide read-only RBAC binding grants the Jenkins service account access only to read Node readiness. Inspect or recover manually:

```bash
kubectl -n jenkins get lease playsay-ci-capacity
kubectl -n jenkins get configmap playsay-ci-capacity-state -o yaml
kubectl -n jenkins get cronjob,jobs -l app.kubernetes.io/part-of=playsay-ci

# Recovery only, using the checked-out platform script from a Jenkins tools pod:
CI_CAPACITY_FORCE_RESTORE=true ./scripts/ci/manage-build-capacity.sh restore
```
...
The Kubernetes secret `playsay-openai` contains exactly one data key, `api-key`. Dev and prod must use separate OpenAI Platform projects and separate keys; never copy the dev key into prod. The owner confirmed interactive installation of the independent prod key on 2026-07-21. Production root was manually promoted to `release/1.001.02`; `api-gateway` and `vocabulary-service` were synced and reported `Synced/Healthy`. Their live Deployment configuration reports provider `openai` and model `gpt-5.6-sol`. AI Tutor and lesson translation remain unchanged. Functional acceptance still requires one authenticated material-generation smoke and one vocabulary-suggestion smoke.
