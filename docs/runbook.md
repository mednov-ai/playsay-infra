...
Enable or roll back dev entry by changing only `honey_school_geoip_redirect_dev_enabled` in Git and repeating the exact-ref play. Production stays false. Disable the update timer independently to pause downloads; preserve the last verified database and its age limit. Restore a validated last-good database through the reviewed updater rollback procedure; never edit credentials to simulate provider failure.
