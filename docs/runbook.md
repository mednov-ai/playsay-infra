...
The current VDSina procedures below remain authoritative until the migration is explicitly completed. AX41 implementation is active at `65.109.55.110`: the Ubuntu 24.04 physical host, healthy mdadm RAID1/ext4, KVM/QEMU/libvirt, OpenTofu, firewall, WireGuard and VPN-only Cockpit baseline are active. The NAT-backed `playsay-prod` (`10.60.0.20`, 8 vCPU/42 GiB) and `playsay-dev` (`10.60.0.30`, 2 vCPU/12 GiB) Ubuntu VMs are running with verified SSH, cloud-init and QEMU guest agents. Kubernetes, application/data restore, edge TLS and traffic cutover are still pending. The target uses one public IPv4 with NAT, VPN-only management and later off-host Storage Box backups; it does not require Proxmox or ZFS. OpenTofu plan hashes and verified encrypted state evidence are recorded in `migrations/ax41/evidence/20260720-opentofu-vms.md`.
...
Initial safety capture `playsay-safety-v2-20260720T201827Z` completed on 2026-07-20. It contains consistent PostgreSQL 17 custom dumps for the application and full Keycloak database, the MinIO data archive, both Sealed Secrets recovery keys and sanitized inventories. Transport and every payload checksum passed; both dump catalogs were validated with source PostgreSQL 17 tools and a local decrypt/archive verification succeeded. The encrypted files are off-host at `/Users/evgeniymednov/Backups/PlayAndSay/ax41-migration-20260720`; the RSA private key is stored separately at `/Users/evgeniymednov/.ssh/play_and_say_migration_backup_rsa.pem`. Do not delete the private key, place it in Git or copy it next to the bundle. This is an initial disaster-recovery capture, not the later selective prod seed or final cutoff bundle.
