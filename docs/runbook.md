...
AX41 at `65.109.55.110` serves `honey.school` production from the current numeric release: the Ubuntu 24.04 physical host, healthy mdadm RAID1/ext4, KVM/QEMU/libvirt, OpenTofu, firewall, WireGuard and VPN-only Cockpit baseline are active. The NAT-backed `playsay-prod` (`10.60.0.20`, 8 vCPU/38 GiB), `playsay-dev` (`10.60.0.30`, 2 vCPU/10 GiB) and `playsay-ci` (`10.60.0.40`, 2 vCPU/8 GiB) guests are active and protected by separate OpenTofu states. Prod/dev run independent k3s, ArgoCD and Sealed Secrets controllers. CI runs only k3s, Sealed Secrets and the Git/JCasC-defined Jenkins controller; it has no product workloads, ArgoCD, Keycloak or MinIO. Prod has exactly seven human identities, 37 Maria-owned materials, 110 verified material assets/objects and 24 HTML-game enrichments, with `hello` and legacy lesson/assignment/collaboration history excluded from the material delta. All 15 prod ArgoCD apps are `Synced/Healthy`; web, keyboard, API, collaboration signaling, LiveKit signaling and forced TURN reach AX41. Canonical Keycloak is `ops.*/keycloak`; the explicit `key.*/keycloak` compatibility path remains only until owner-operated login/material acceptance. Both GitHub webhooks now use `hooks.honey.school` and returned HTTP 200 to GitHub ping deliveries. Email/payment provider workloads remain disabled until independent prod credentials are supplied. Object Storage remains a post-stabilization state/backup task, not a blocker to this cutover. Initial cutover evidence is `migrations/ax41/evidence/20260721-honey-cutover-and-final-backup.md`; the current material inventory is recorded in `migrations/ax41/evidence/20260726-maria-material-delta.md`.
...
The old VPS has only the short paid overlap remaining. Full source bundle `playsay-final-vps-v2-20260721T083819Z` has been copied off the VPS and fully decrypt/checksum/archive verified. Maria continued authoring there after that cutoff; the targeted `20260726T102149Z` material delta is now encrypted off-host and applied to production with matching database and object hashes. Keep the VPS and old Jenkins available only as rollback through owner acceptance; delete neither without explicit owner approval. Amnezia is not migrated and ends with the VPS. AX41 administration uses its independent WireGuard management VPN.

### Maria material delta from the retired VPS

The 2026-07-26 delta added 15 materials, refreshed three existing materials, and added 59 asset rows/MinIO objects plus 13 HTML-game enrichment rows. Production now matches the retired VPS for Maria's reusable library at `37/110/24`; the test material `hello`, lesson/assignment/submission/collaboration history, annotations and derived YouTube cache rows remain excluded.

Any repeat delta must use the same safety gates: capture an encrypted production PostgreSQL backup first, take a stable source signature, copy only absent object keys through authenticated S3 operations, verify each destination object by SHA-256, then merge the database rows in one transaction that refuses unexpected production changes. Never copy MinIO PVC files directly. Recompute the source signature after object transfer and after the transaction; a changed signature requires another reviewed delta rather than an unbounded full restore. Non-secret evidence and current checksums are in `migrations/ax41/evidence/20260726-maria-material-delta.md`.
...
The overload incidents above are retained as the reason for isolating CI. They are historical evidence, not instructions for the AX41 deployment. On the old VPS, leave any already-running watchdog resources untouched until the rollback window closes; do not recreate them on dev, prod or CI.
...
The app DB migrate stages live in `playsay-api-gateway-develop`, `playsay-payment-service-develop`, `playsay-registration-service-develop`, and `playsay-email-service-develop`; AI Tutor, Vocabulary and Keyboard use the same launcher in their module jobs. They run after module test/package and before image build for deployable branches, and they skip when `GITHUB_BEFORE..GITHUB_AFTER` proves the module's `db/changelog` directory did not change. Missing/invalid diff metadata fails safe to running the migration. Jenkins itself does not mount DB Secrets. It uses the scoped dev kubeconfig to create a short-lived `playsay-migrate-*` Job and `playsay-migration-*` ConfigMap in `playsay-dev`; only that Job may read one of the approved dev DB Secrets. Admission policy fixes the Liquibase `5.0.3` image, PostgreSQL JDBC `42.7.8`, command, volumes, security context and allowed Secret keys. The Job has no service-account token, is time-bounded, and is deleted with its generated ConfigMap after completion. Keep service startup Liquibase disabled in Helm; migrations are controlled by Jenkins, not by service boot.
...
The model is ordinary reviewed configuration in Git, not a credential. Environment-specific Helm values define it independently for material drafts and vocabulary, with matching defaults per environment:
...
Before enabling prod, create `release/<version>.<subversion>.<patch>` from the accepted release, set both prod flags below to `true`, run Helm validation, commit the change, then manually sync the two ArgoCD applications:

```yaml
# helm-charts/api-gateway/values-prod.yaml
ai:
  openai:
    enabled: true
    model: gpt-5.6-sol

# helm-charts/vocabulary-service/values-prod.yaml
openai:
  enabled: true
  model: gpt-5.6-sol
```

Do not enable these flags before the prod Secret exists: the deployments intentionally fail closed when `api-key` is missing. After deploying the chart, verify provider and model names without printing credentials:

```bash
KUBECONFIG=/etc/rancher/k3s/k3s.yaml kubectl -n "$TARGET_NAMESPACE" get deploy api-gateway \
  -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="PLAYSAY_AI_PROVIDER")].value}{"\n"}{.spec.template.spec.containers[0].env[?(@.name=="OPENAI_MODEL")].value}{"\n"}'
KUBECONFIG=/etc/rancher/k3s/k3s.yaml kubectl -n "$TARGET_NAMESPACE" get deploy vocabulary-service \
  -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="PLAYSAY_VOCABULARY_OPENAI_MODEL")].value}{"\n"}'
```

Expected values after prod activation are `openai`, `gpt-5.6-sol`, and `gpt-5.6-sol`. Run an authenticated material-generation smoke and a vocabulary suggestion smoke, watch provider errors/rate limits, and roll back by setting both flags to `false` in a new release patch if either path is unstable.
