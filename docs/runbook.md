...
The Jenkins controller is not yet present on AX41; the active controller and its credential store remain on the retiring VPS. Do not delete the VPS until a clean Jenkins controller is installed from this repository on dedicated guest `playsay-ci`, fresh/re-authorized GitHub credentials and webhooks are configured, one affected-target build reaches GHCR/Git/ArgoCD successfully, and the old webhook/controller is disabled. Do not cold-copy the old Jenkins PVC or expose the replacement UI publicly; use the management VPN at `jenkins.honey.school`. Only the two restricted webhook endpoints on `hooks.honey.school` are public.
