...
Jenkins chart must keep `controller.overwritePlugins=true`. With the chart default `false`, a controller restart can leave the init container blocked on interactive `cp -i` plugin overwrite prompts and Jenkins will serve `502` through host nginx because the service has no ready endpoints.
