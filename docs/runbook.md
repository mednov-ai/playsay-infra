...
The current VDSina procedures below remain authoritative until the migration is explicitly completed. AX41 implementation has started at `65.109.55.110`: the Ubuntu 24.04 physical host, healthy mdadm RAID1/ext4, KVM/QEMU/libvirt, OpenTofu, firewall, WireGuard and VPN-only Cockpit baseline are active, but the two workload VMs and DNS cutover do not exist yet. The target uses two isolated VMs managed by OpenTofu, one public IPv4 with NAT, VPN-only management and off-host Storage Box backups; it does not require Proxmox or ZFS.
