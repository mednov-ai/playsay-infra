...
The live stage activated on 2026-07-25 owns `honeyschool.ru`, `www.honeyschool.ru` and the public Keycloak ingress at `ops.honey.school`. The REG.RU A records for `honeyschool.ru` and `www` point to Selectel and proxy the existing static `honey.school` landing from AX41. Because the AX41 landing certificate does not contain the root `honey.school` name, this route verifies upstream TLS with SNI `online.honey.school` while preserving HTTP Host `honey.school`; upstream TLS session reuse is disabled so certificates cannot be reused across different SNI routes. Its independent Let's Encrypt certificate is stored at `/etc/letsencrypt/live/honeyschool.ru` and expires on 2026-10-23.
...
The remaining browser cutover is still gated: `online.honey.school` and `key.honey.school` stay directly on AX41 until forced-resolution smoke through Selectel returns HTTP 200 for each complete frontend bundle. `dev.*`, `jenkins.honey.school` and `hooks.honey.school` also remain directly on AX41. The public Keycloak route has passed forced-resolution and post-DNS discovery, PKCE login-theme resource, TLS 1.2 and restricted-path smoke. On 2026-07-25 the owner confirmed that the Keycloak account/login page opens from an affected Russian connection without VPN, so the Selectel Keycloak ingress is accepted.
...
Do not copy the AX41 private certificate key to Selectel. `ops.honey.school` already uses its independent Selectel ACME certificate and unattended HTTP renewal. When `online.honey.school` and `key.honey.school` are ready, issue reviewed Selectel ACME certificates for those names, verify unattended renewal and split them out of the next AX41 SAN renewal. Keep the canonical production issuer `https://ops.honey.school/keycloak/realms/playsay`; moving an ingress IP must not change the issuer string.
