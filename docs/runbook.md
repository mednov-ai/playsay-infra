...
Normal cleanup runs in Jenkins `post always`. The `playsay-ci-capacity-watchdog` CronJob runs every minute and restores saved replicas when the agent pod disappears or the 40-minute deadline expires. The `capacity-guard` sidecar deletes the agent pod immediately on NodeNotReady or after four 15-second samples with `MemAvailable <1GiB` or I/O wait `>30%`; watchdog then restores services. Inspect or recover manually:
